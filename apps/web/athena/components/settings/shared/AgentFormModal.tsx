/**
 * Copyright (c) 2023-present Plane Software, Inc. and contributors
 * SPDX-License-Identifier: AGPL-3.0-only
 * See the LICENSE file for details.
 *
 * Registering an agent names one that is already running, so a mention can reach it.
 *
 * It used to also ask for a provider and a model. Neither decided anything: the model that answers
 * comes from the workspace's own provider settings, resolved per mention by the runtime, so two
 * agents registered with different models both answered on the workspace's. The fields are gone
 * rather than wired, because one process answers for the workspace and a per-agent model would be
 * a second, quieter place to configure the same thing.
 *
 * Create-only — POST /chat/settings/agents has no identity-changing PATCH (only `enabled`, handled
 * inline in AgentsPanel's toggle), so this modal never takes an existing agent to edit.
 */
import { useState } from "react";
import { Button, EModalPosition, EModalWidth, Input, ModalCore } from "@plane/ui";
import { FOCUS_RING } from "../../../utils/focusRing";
import type { CreateAgentInput } from "../types";

/** A Nostr public key is 64 hex characters — checked here so a typo is caught before the save. */
const PUBKEY_PATTERN = /^[0-9a-f]{64}$/i;

interface AgentFormModalProps {
  readonly isOpen: boolean;
  readonly onClose: () => void;
  readonly onSubmit: (values: CreateAgentInput) => Promise<{ ok: boolean; error?: string }>;
}

const EMPTY_VALUES: CreateAgentInput = { name: "", pubkey: "" };

export function AgentFormModal(props: AgentFormModalProps) {
  const { isOpen, onClose, onSubmit } = props;
  const [values, setValues] = useState<CreateAgentInput>(EMPTY_VALUES);
  const [submitting, setSubmitting] = useState(false);
  const [error, setError] = useState<string | null>(null);

  function resetAndClose() {
    setValues(EMPTY_VALUES);
    setError(null);
    onClose();
  }

  async function handleSubmit() {
    if (values.name.trim().length === 0) {
      setError("Give the agent a name — it is what people will @mention.");
      return;
    }
    if (!PUBKEY_PATTERN.test(values.pubkey.trim())) {
      setError("The public key should be the 64-character key from the agent's startup line.");
      return;
    }
    setSubmitting(true);
    setError(null);
    const result = await onSubmit({ name: values.name.trim(), pubkey: values.pubkey.trim().toLowerCase() });
    setSubmitting(false);
    if (result.ok) resetAndClose();
    else setError(result.error ?? "Something went wrong.");
  }

  return (
    <ModalCore isOpen={isOpen} handleClose={resetAndClose} position={EModalPosition.CENTER} width={EModalWidth.LG}>
      <div className="flex flex-col gap-4 p-5">
        <h3 className="text-16 font-semibold text-primary">Register an agent</h3>
        <p className="text-13 text-secondary">
          This makes an agent @mentionable in chat. It names one that is already running — its public key is how a
          mention reaches it. Which model it answers with is this workspace&apos;s setting, under Model.
        </p>

        <div className="flex flex-col gap-1.5">
          <label htmlFor="agent-name" className="text-13 font-medium text-primary">
            Name
          </label>
          <Input
            id="agent-name"
            value={values.name}
            onChange={(e) => setValues((v) => ({ ...v, name: e.target.value }))}
            placeholder="e.g. athena-agent"
            maxLength={80}
            className={FOCUS_RING}
          />
        </div>

        <div className="flex flex-col gap-1.5">
          <label htmlFor="agent-pubkey" className="text-13 font-medium text-primary">
            Public key
          </label>
          <Input
            id="agent-pubkey"
            value={values.pubkey}
            onChange={(e) => setValues((v) => ({ ...v, pubkey: e.target.value }))}
            placeholder="64-character key from the agent's startup line"
            maxLength={64}
            className={`font-mono ${FOCUS_RING}`}
          />
          <span className="text-11 text-tertiary">
            Printed by the agent runtime as <span className="font-mono">pubkey=…</span> when it starts.
          </span>
        </div>

        {error && <p className="text-12 text-danger-primary">{error}</p>}

        <div className="mt-1 flex justify-end gap-2">
          <Button variant="neutral-primary" size="sm" onClick={resetAndClose} disabled={submitting}>
            Cancel
          </Button>
          <Button
            variant="primary"
            size="sm"
            onClick={() => void handleSubmit()}
            loading={submitting}
            disabled={submitting}
          >
            Register agent
          </Button>
        </div>
      </div>
    </ModalCore>
  );
}
